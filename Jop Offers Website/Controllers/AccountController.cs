...
namespace WebApplication1.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public ActionResult Register()
        {
            ...
            return View();  //من حق اليوزر يختار اي رول عدا الادمنستراتور
        }
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                //هنعبأ الدروب داون ليست من جديد
                ViewBag.UserType = new SelectList(db.Roles.Where(a => !a.Name.Contains("Adminstrators")).ToList(), "Name", "Name");

                //ابليكاشن يوزر ده كلاس بيمثل اليوزر هنكريت منه اوبجكت
                var user = new ApplicationUser { UserName = model.UserName, Email = model.Email, UserType = model.UserType };
                var result = await UserManager.CreateAsync(user, model.Password);//كريت اليوزر وجطه فى المتعير ريزلت
                if (result.Succeeded)//لو اتكريت اليوزر وتمام اعمل ساين ان
                {
                    ...
                    //جدول اليوزر رول هنضيف فيه الرول بتاعة كل يوزر (m:m table)
        ...
        {
            var UserId = User.Identity.GetUserId(); //جبنا الايدى بتاع اليوزر الحالى
        ...
        public ActionResult EditProfile(EditProfileViewModel profile)
        {
            ...
            else
            {
                ...
                //حولنا الباسورد الجديد اللى دخله ليوزر لهاش باسورد اي لرمز لانه هيتسجل فى الداتا بيز كهاش اى كرمز
